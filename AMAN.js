...
let myuser_entries=[];
if(localStorage.getItem("myuser_entries")===null)
{
    myuser_entries =[];
}
else{
   myuser_entries = JSON.parse(localStorage.getItem("myuser_entries")); 
}
let errors=[]
const retieveEntries = ()=>{
let entries = localStorage.getItem('myuser_entries')
if(entries){
...
if(age < 19 || age > 50){
  ...
  return  alert("Age must be between 18 and 55 or else you will be undefined")
    ...
    const entry ={
     ...
     myuser_entries.push(entry);
     localStorage.setItem("myuser_entries",JSON.stringify(myuser_entries))
    displayEntries()
    userForm.reset() 
}
}
